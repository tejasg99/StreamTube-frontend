...
import { useGetChannelSubscribers } from "../../hooks/subscription.hook";
import { useSelector } from "react-redux";
import { Subscriber, SubscriberSkeleton } from "../../components/index";

function ChannelSubscribers() {
  const channelId = useSelector((state) => state.channel.channel?._id);
  const [searchTerm, setSearchTerm] = useState("");
  const { data: channelSubscribers, isLoading } =
    useGetChannelSubscribers(channelId);
    ...
    return (
      <div className="flex justify-center p-4 mt-4">
        <div className="w-full max-w-sm text-center">
          ...
          <p>This channel has no subscribers yet</p>
        </div>
      </div>
    );
  }
